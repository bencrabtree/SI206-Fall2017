...
## Test that the return value of the play_war_game function is a
## tuple with three elements, the first of which is a string. (This will probably require multiple test methods!)
...
## Make sure to include a descriptive message in these two so we can easily see what you are testing!
	...
	def test_myTestInstance1(self):
		d = Deck() # initalize the deck
		d.pop_card() # remove a card from the deck
		self.assertEqual(len(d.cards), 51) # this is testing if we call pop_card if it actually removes it from the deck (len would be 52 - 1 = 51)
